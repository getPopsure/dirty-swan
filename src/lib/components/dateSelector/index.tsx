...
export const DateSelector = ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      const { isValid, errorType } = isDateValid(
        value,
        yearBoundaries,
        calendarDateValue
      );
  ...
  const handleOnChange = (key: keyof CalendarDate, v?: string) => {
    ...
    const isDateInValidFormat = dayjs(
      formattedDate,
          ...
          {
            {
              ...
              afterMax:
                placeholders.errorAfterMaxYear ||
                `Please choose a date before ${yearBoundaries.max + 1}`,
              beforeMin:
                placeholders.errorBeforeMinYear ||
                `Please choose a date after ${yearBoundaries.min - 1}`,
            }[hasError || 'default']
          }
        </p>
      )}
