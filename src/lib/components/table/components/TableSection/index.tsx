  ...
  return (
    <div style={{ width: tableWidth }}>
      {data.map(({ items, section = {} }, index) => {
        ...
        const isExpanded = !collapsibleSections ? true : isSectionOpen === index || isFirstSection;
        const isVisible = hideDetails ? !shouldHideDetails : true;

        return (isFirstSection || isVisible) && (
          <div key={index}>
            {section?.title && (
              <div className={styles.cardWrapper}>
                <div className={classNames(styles.card, 'p8')}>
                  <Card
                    actionIcon={isExpanded
            ...
            >
              <TableContent
                ...
                hideHeader
                openModal={openModal}
                title={`${title}${section?.title ? ` - ${section.title}` : ''}`}
                width={tableWidth}
              />
            </Collapsible>
          </div>
