import { Meta, Preview } from '@storybook/addon-docs/blocks';

import SuggestMultiSelect from '.';

<Meta title="JSX/SuggestMultiSelect" />

## SuggestMultiSelect

This component allows quick search via the input field to find an option for selection.

Upon selecting an option, the option is displayed above the input field as a tag.

The tag can be removed by clicking on the X button. Multi options can be selected as well.

Prop `setValues` function must be provided to keep track of removed and added option selections.

## Examples

### Default

<Preview>
  <SuggestMultiSelect
    options={[
      { value: 'This is a test option' },
      { value: 'Here is another test option' },
      { value: 'Last test option' },
    ]}
    selectedValues={[{ value: 'This is a test option' }]}
    setValues={() => {}}
    placeholder="Placeholder"
  />
</Preview>
